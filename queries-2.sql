...
-- CORREÇÃO #1 — contrato 102 (Carlos Lima / Consignado):
-- A parcela 1004 está em aberto (data_pagamento NULL) desde 2023-05-01.
-- Isso caracteriza inadimplência pela Res. CMN 2.682/1999.
-- O status foi corrigido de 'Ativo' para 'Inadimplente' para refletir
-- a realidade dos dados e manter consistência com o README.
INSERT INTO contratos VALUES
    (101, 1, 'Crédito Pessoal', 15000.00,  '2023-01-01', '2024-01-01', 'Quitado'),
    (102, 2, 'Consignado',      28000.00,  '2023-03-01', '2025-03-01', 'Inadimplente'), -- corrigido
...
INSERT INTO pagamentos VALUES
    ...
    (1004, 102, '2023-05-01', NULL,         1166.67,  NULL,     NULL),  -- parcela em aberto
...
-- CORREÇÃO #2 — Exposição financeira em risco por UF
-- Versão original filtrava apenas WHERE data_pagamento IS NULL,
-- ignorando pagamentos parciais (ex.: João Ferreira pagou R$600 de R$708,33).
-- Com isso, a UF 'BA' sumia dos resultados apesar de ter R$108,33 em aberto.
-- Correção: expor toda parcela com saldo residual > 0, independentemente de
-- data_pagamento ser NULL ou não.
SELECT
    cl.uf,
    SUM(p.valor_devido)                                      AS total_devido,
    SUM(COALESCE(p.valor_pago, 0))                           AS total_recebido,
    SUM(p.valor_devido - COALESCE(p.valor_pago, 0))          AS exposicao_risco
FROM pagamentos  p
JOIN contratos   ct ON p.contrato_id = ct.contrato_id
JOIN clientes    cl ON ct.cliente_id = cl.cliente_id
WHERE p.valor_devido - COALESCE(p.valor_pago, 0) > 0    -- inclui pagamentos parciais
GROUP BY cl.uf
...
-- CORREÇÃO #3 — AVERAGEIF entre tabelas de granularidades diferentes
-- (base_pagamentos tem 8 linhas; contratos tem 5) causava resultado errado no Excel.
-- No SQL a query já estava correta ao fazer JOIN diretamente em contratos;
-- a nota abaixo documenta que no Excel.md a fórmula foi corrigida (ver excel.md).
SELECT
    cl.segmento,
    COUNT(DISTINCT ct.contrato_id)        AS total_contratos,
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN p.dias_atraso > 0 THEN p.pagamento_id END)      AS parcelas_atrasadas,
    ROUND(
